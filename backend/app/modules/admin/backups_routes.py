...
def _validate_backup_integrity(tenant_id: uuid.UUID, table_rows: dict[str, list[dict]]) -> None:
    tables = _table_by_name()
    _validate_row_level(tenant_id, table_rows, tables)
    _validate_foreign_keys(table_rows, tables)
...
async def _enqueue_scheduled_job(actor: str) -> BackupJobEntry | None:
    async with _jobs_lock:
        jobs = _load_jobs()
...
async def _scheduler_loop() -> None:
    ...
    while True:
        await _enqueue_scheduled_job(actor="scheduler")
...
async def admin_restore_backup(
    ...
    _validate_backup_integrity(uuid.UUID(match["tenant_id"]), table_rows)
