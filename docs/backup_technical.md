...
1. **Job-Queue**: Persistenter Worker/Planner (z. B. Cron/Scheduler) für wiederkehrende Batch-Backups.
2. **Storage-Abstraktion** als mögliche Lösung (z. B. Provider-Interface für alternative Backends).
3. **Validierung**: optionale Checks wie row-level Checksums oder referenzielle Integrität.
